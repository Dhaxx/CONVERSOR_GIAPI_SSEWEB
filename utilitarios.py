...
def acerta_interessado():
    print("Acertando contribuintes...")
    cur_dest.execute("""SELECT DISTINCT(a.CODIGO_PRT),a.NOME_INT, b.COD_CNT  FROM SE_PPROTOCOLO a INNER JOIN GR_CONTRIBUINTES b on a.NOME_INT = b.NOME_CNT
                        AND EXERCICIO_PRT >= '2013'
                        ORDER BY CODIGO_PRT""")
    update = cur_dest.prep("""UPDATE SE_PPROTOCOLO SET INTERESSADO_PRT = ? WHERE CODIGO_PRT = ?""")

    for row in cur_dest.fetchall():
        interessado_prt = row[2] 
        codigo_prt = row[0]
        cur_dest.execute(update, (interessado_prt, codigo_prt))
    cnx.commit()